...
def minimize_marginals(graph, initial_estimate, pose_options):
    ...
    for poseKey, poseVal in pose_options.items():
            for landmarkIDx in [1, 2]:
                ...
                marginals = gtsam.Marginals(testGraph, finalResults)
                sumCurrent = marginals.marginalCovariance(L(landmarkIDx)).sum()


                marginalsObj = gtsam.Marginals(testGraph, finalResults)
                sumMarCurrent = (marginalsObj.marginalCovariance(L(1)).sum() + 
                           marginalsObj.marginalCovariance(L(2)).sum())
                ...
                if sumCurrent < lowestSum:
                    ...
                    sum_of_marginals = sumMarCurrent
